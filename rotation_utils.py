...
def random_orthogonal_matrix(size, device, seed=None):
    """
    ...
    device: The device to create the matrix on.
...
def get_orthogonal_matrix(size, mode, device=utils.DEV, seed=None):
    if mode == 'random':
...
def rotate_model(model, args):
    Q = get_orthogonal_matrix(model.config.hidden_size,
                                                args.rotate_mode,
                                                seed=args.rotation_seed if args.rotation_seed >= 0 else None)
    config = model.config
